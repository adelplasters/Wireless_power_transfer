...
function [M,k] = mutual_inductance(Rx,Tx,caso)
    ...
    b = 0.7e-2; %Rx radius
    mu0 = (4*pi)*1e-7;
    delta = 10e-2; %centre - centre distance
    ...
    %lateral misalignment 
        case 'lateral'
               for i=1:100
                ...
                Nlat = @(phi) sqrt(4*a*blat(phi))/(((a + blat(phi))^2) + dist(i)^2);
        ...
        %Glat = @(phi) ((2/Nlat(phi)) - Nlat(phi))*Klat - (2/Nlat(phi))*Elat;
                ...
                M = factor1*integral2(intfun,0,2*pi,0,2*pi,'RelTol',1e-9,'AbsTol',1e-14,'method','iterated');
